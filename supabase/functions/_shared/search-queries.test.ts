...
beforeAll(async () => {
  (globalThis as unknown as { Deno: { env: { get: () => undefined } } }).Deno =
    { env: { get: () => undefined } };
  const mod = await import("../_shared/config.ts");
...
describe("getAllSearchQueries", () => {
  // PREPIO-77 follow-up: with `enableHybridScraping` off, `searchCompanyInfo`
  // is the only retrieval path. It caps query breadth with `.slice(0, 6)` to
  // stay under the 15s function timeout. If the categories are emitted in
  // blocks (all 4 Glassdoor first, then Blind, etc.), that slice never
  // touches Blind/Reddit/LeetCode/international — defeating the purpose of
  // flipping the hybrid path off.
