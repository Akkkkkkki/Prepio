...
const sortCitableBullets = (bullets: ProfileBullet[]) =>
  [...bullets]
    ...
    .sort((left, right) => {
      ...
      const lengthDifference = clean(right.text).length - clean(left.text).length;
      return lengthDifference || clean(left.id).localeCompare(clean(right.id));
    });
...
export function serializeProfileForPrompt(
  ...
  const maxProjects = opts.maxProjects ?? DEFAULT_PROJECT_CAP;
  ...
  const addBullet = (bullet: ProfileBullet, sourceLabel: string) => {
    ...
    if (!realBulletId || !text || bulletIndex[realBulletId]) return false;
  ...
  (profile.projects || []).slice(0, maxProjects).forEach((project) => {
  ...
  const starLine = () => {
    const aliases = Object.values(bulletsByAlias)
...
export function resolveStoryForQuestion(
  question: QuestionForStoryMatch,
  profileContext: ProfilePromptContext | null | undefined,
  stageThemes: string[] = [],
): SerializedStoryBullet | null {
  if (clean(question.leveragesStoryId)) {
    // A hallucinated model alias degrades to null. The deterministic matcher
    // is deliberately reserved for questions where the model returned null.
    return resolveStoryAlias(question.leveragesStoryId, profileContext);
  }
  return matchStoryForQuestion(question, profileContext, stageThemes);
}
