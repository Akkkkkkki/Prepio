...
COMMENT ON COLUMN answer_feedback.generation_metadata IS
  'Includes model/runtime details and the exact input snapshot used for generation.';
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_answer_feedback_current
  ON answer_feedback(practice_answer_id)
  WHERE superseded_by IS NULL AND practice_answer_id IS NOT NULL;

DROP TRIGGER IF EXISTS answer_feedback_updated_at ON answer_feedback;
...
DROP POLICY IF EXISTS answer_feedback_own_read ON answer_feedback;
CREATE POLICY answer_feedback_own_read ON answer_feedback FOR SELECT
  USING (auth.uid() = user_id);
DROP POLICY IF EXISTS answer_feedback_service ON answer_feedback;
