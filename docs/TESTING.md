...
`npm run lint` is informational, not a release gate. As of 2026-07-05 (after the eslint 10 / eslint-plugin-react-hooks 7 upgrade) it reports **15 problems (7 errors, 8 warnings)**. This section triages what's there so reviewers can tell at a glance whether a new lint hit is signal or noise.
