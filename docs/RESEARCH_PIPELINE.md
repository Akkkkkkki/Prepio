...
| 4 | Retrieval was SWE-biased regardless of role, and the DuckDuckGo "fallback" hit the instant-answer API and returned no forum results. **FIXED:** `buildResearchQueryPlan` classifies role-family and lets `level`, `country`, and the user note shape queries and domain packs; the DuckDuckGo path is gone and nothing is substituted for a failed search. **Caveat on the telemetry:** `searchTavilyWithDeduplication` catches HTTP, network and JSON-parse failures and returns `null` (`_shared/tavily-client.ts`), so `company-research` sees an empty result and logs `TAVILY_SEARCH_EMPTY` for both a genuine zero-result query and a Tavily outage. The `TAVILY_SEARCH_FALLBACK_UNAVAILABLE` branch is effectively unreachable on those paths. The events cannot currently distinguish "no results" from "retrieval broke" — disambiguating them is a separate, still-open follow-up that means editing `company-research`'s discovery logging (a service-role edge function, so it needs explicit sign-off) and is out of scope for the shim removal. The dead `_shared/duckduckgo-fallback.ts` shim and its test were deleted in PREPIO-155. | `company-research/query-planner.ts` | PREPIO-80 ✓ (shim deleted: PREPIO-155 ✓) |
| 5 | `job-analysis` fell back to generic stub requirements with no provenance flag; synthesis was told they came "from link analysis". **FIXED:** the payload carries the snake_case wire field `requirements_source: "extracted" \| "stub"` (the orchestrator reads exactly that key; the camelCase form does not exist on the wire), a stub run reports no sources, and a `[job-analysis] stub-fallback` log line makes the stub rate countable. | `job-analysis/index.ts` | PREPIO-82 ✓ |
...
|------|----------|--------|-------|
| 0. Stop the bleeding | Flag job-analysis stubs; remove the no-op DuckDuckGo fallback | PREPIO-82, PREPIO-80, PREPIO-155 | **Done.** The dead `duckduckgo-fallback.ts` shim was deleted in PREPIO-155 ✓ |
