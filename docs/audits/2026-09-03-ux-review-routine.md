...
- **Keyboard focus is visible; 200% zoom holds.** Tab order on landing is Skip-link → nav → Pricing →
  "Sign in or create account" → form inputs, each interactive control with a visible focus ring (2px
  solid outline + box-shadow ring); `body{zoom:2}` produces no horizontal overflow (1440/1440).
...
| Accessibility | 3 | **4** | **▲** | **(live)** Practice screen now has `<h1>` on desktop **and** mobile (#329), focus rings visible, redirect context preserved, 200% zoom no overflow. Remaining: `/auth` autocomplete null (#3), sub-44px landing/auth targets (#5). |
