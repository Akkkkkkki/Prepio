...
- Post-answer rubric self-check for free users (PREPIO-41) and follow-up question
  drilling / "interviewer mode" (PREPIO-47). **The UI for both shipped; on a fresh research
  run neither can render.** `interview-research` hardcodes `evaluation_criteria: []` and
  `follow_up_questions: []` and never writes `good_answer_signals` at all
  ([`index.ts:1030-1032`](../supabase/functions/interview-research/index.ts)), so the fields
  these features read are empty on anything generated by the current pipeline. Tracked in
  PREPIO-176.
