...
- Post-answer rubric self-check for free users, reusing each question's
  `good_answer_signals` / `evaluation_criteria` (PREPIO-41).
- Follow-up question drilling ("interviewer mode") off each question's
  `follow_up_questions` (PREPIO-47).
...
- **State as of 2026-08-29:** the "Your interviews" home, the switcher deletion, the
  de-densified Plan, one-tap practice, and the design-system lock have all shipped. Two
...
- **Deploy the backend to parity with `main` (PREPIO-124, Urgent).** Production has been
  ...
  preview, billing, paid feedback, CV import, and voice are dead in production regardless of
  what this file says is shipped. Everything else below is downstream of this.
