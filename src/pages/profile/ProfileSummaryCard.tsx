...
import { ArrowRight, Link2, MapPin, Sparkles, Trash2, Upload } from "lucide-react";
...
import { createEmptyProfileLink, getCandidateProfileUnlock } from "@/lib/candidateProfile";
...
const ProfileSummaryCard = ({
  ...
  const unlock = getCandidateProfileUnlock(profile);
    ...
    <Card className="overflow-hidden border-border/70 bg-gradient-to-br from-background via-background to-muted/30">
      <CardContent className="grid gap-6 p-6 lg:grid-cols-[minmax(0,1.7fr)_280px]">
        ...
        <div className="space-y-4 rounded-[20px] border border-border/70 bg-background/90 p-5">
          <div className="space-y-2">
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm font-medium">Profile unlocks</p>
              <Badge variant="secondary">{profile.completionScore}%</Badge>
            </div>
            <Progress value={profile.completionScore} />
            <p className="text-xs text-muted-foreground">
              {unlock ? "Next highest-value action for downstream prep." : "Core profile unlocks are ready for research and practice."}
            </p>
          </div>

          <div className="space-y-3 rounded-2xl bg-muted/40 p-4 text-sm">
            <p className="font-medium">{unlock ? "Next unlock" : "Unlocked"}</p>
            <p className="text-foreground">{unlock?.copy ?? "Research, dashboard gaps, and practice matching can use this profile."}</p>
            <p className="text-xs text-muted-foreground">
              {unlock?.payoff ?? "Keep refining it when your target role changes."}
            </p>
            {unlock ? (
